...
function createNycParameter(option, command) {
    let commandLine = ' ';
...
if (options.uploadCartridge) {
    ...
    cartridges.forEach(cartridge => {
        exitCode = shell.exec(shellCommands('--cartridge', cartridge));
        if (exitCode && exitCode.code) {
            console.error(`Problem uploading cartridge : ${cartridge}`);
            process.exit(1);
        }
    });
...
if (options.cover) {
    const nyc = fs.existsSync(path.resolve(cwd, './node_modules/.bin/nyc'))
        ? path.resolve(cwd, './node_modules/.bin/nyc')
        : path.resolve(pwd, './node_modules/.bin/nyc');
    const mocha = fs.existsSync(path.resolve(cwd, './node_modules/.bin/_mocha'))
        ? path.resolve(cwd, './node_modules/mocha/bin/_mocha')
        : path.resolve(pwd, './node_modules/mocha/bin/_mocha');

    const subprocess = spawn(
        `${nyc} -r text -r lcov ${createNycParameter(options.exclude, 'x')} ${createNycParameter(options.include, 'n')} ${mocha}  -- -R spec --recursive "${options.cover.join(' ')}"`,
        { stdio: 'inherit', shell: true, cwd }
    );
...
if (options.lint) {
    if (options.lint === 'js') {
        const subprocess = spawn(path.resolve(cwd, `./node_modules/.bin/eslint${options.fix ? ' --fix' : ''}`)
            + ' .', { stdio: 'inherit', shell: true, cwd });
    ...
    if (options.lint === 'css') {
        const subprocess = spawn(path.resolve(cwd, `./node_modules/.bin/stylelint${options.fix ? ' --fix' : ''}`)
            + ' --custom-syntax postcss-scss "**/*.scss"', { stdio: 'inherit', shell: true, cwd });
